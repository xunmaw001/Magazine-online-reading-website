<template>
    <div class="addEdit-block">
        <el-form
            ...
            <el-row>
                <el-col :span="12"  v-if="sessionTable !='qikanxinxi'">
                    <el-form-item class="select" v-if="type!='info'"  label="期刊" prop="qikanxinxiId">
                        <el-select v-model="ruleForm.qikanxinxiId" :disabled="ro.qikanxinxiId" filterable placeholder="请选择期刊" @change="qikanxinxiChange">
                            <el-option
                                    v-for="(item,index) in qikanxinxiOptions"
                                    v-bind:key="item.id"
                                    :label="item.qikanxinxiName"
                                    :value="item.id">
                            </el-option>
                        </el-select>
                    </el-form-item>
                ...
                <el-col :span="12"  v-if="sessionTable !='qikanxinxi' ">
                    <el-form-item class="input" v-if="type!='info'"  label="标题" prop="qikanxinxiName">
                        <el-input v-model="qikanxinxiForm.qikanxinxiName"
                ...
                <el-col :span="24" v-if="sessionTable !='qikanxinxi' ">
                    <el-form-item class="upload" v-if="type!='info' && !ro.qikanxinxiPhoto" label="图片" prop="qikanxinxiPhoto">
                        <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in (qikanxinxiForm.qikanxinxiPhoto || '').split(',')" :src="item" width="100" height="100">
                    </el-form-item>
                    <div v-else>
            ...
            <input id="qikanxinxiId" name="qikanxinxiId" type="hidden">
...
<script>
    ...
    export default {
        ...
        created() {
         ...
         this.$http({
             url: `qikanxinxi/page?page=1&limit=100`,
             method: "get"
         }).then(({ data }) => {
             if (data && data.code === 0) {
                this.qikanxinxiOptions = data.data.list;
            }
         });
         this.$http({
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `liulanjil/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.qikanxinxiChange(data.data.qikanxinxiId)
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`liulanjil/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.liulanjilCrossAddOrUpdateFlag = false;
                                        this.parent.search();
